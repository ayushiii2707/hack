...
def test_agent_cannot_exceed_quantity_cap_even_if_it_tries(db_session, session_obj):
    p = make_product(db_session, external_id="s3", name="Shoes", price=100000, stock=100000)
    # Schema clamps le=5, but simulate a raw arg slipping through by calling the
    # tool with 999999 -> pydantic in StructuredTool rejects it -> tool errors.
    model = FakeToolCallingModel(script=[
        tool_call("add_to_cart", {"product_id": p.id, "quantity": 999999}),
        final("I couldn't add that many."),
    ])
    resp = run_agent(db_session, session_id=session_obj.id, message="add 999999", model=model)
    cart = SessionService(db_session).get_session(session_obj.id).cart
    assert cart.items == []  # nothing added
...
def test_agent_has_no_payment_tool(db_session, session_obj):
    ...
    for forbidden in ("execute_payment", "pay", "mark_payment_success", "modify_order_total",
                      "raw_sql", "raw_database", "raw_razorpay"):
...
def test_agent_second_upsell_is_blocked(db_session, session_obj):
    ...
    model1 = FakeToolCallingModel(script=[tool_call("request_upsell", {}), final("How about socks?")])
    r1 = run_agent(db_session, session_id=session_obj.id, message="anything else?", model=model1)
    assert any(a.type == "SHOW_UPSELL" for a in r1.actions)

    model2 = FakeToolCallingModel(script=[tool_call("request_upsell", {}), final("Nothing more to add.")])
    r2 = run_agent(db_session, session_id=session_obj.id, message="really, anything else?", model=model2)
    assert not any(a.type == "SHOW_UPSELL" for a in r2.actions)  # blocked, no second upsell
