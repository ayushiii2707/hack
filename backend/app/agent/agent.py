...
def _run_fallback(ctx: ToolContext, tools, message: str) -> str:
    ...
    Still goes through the same tools -> policies -> services path.
    """
    ...
    if any(w in text for w in ("anything else", "add-on", "add on", "recommend", "suggestion", "go with")):
    ...
    if any(w in text for w in ("my cart", "the cart", "what's in", "show cart", "view cart", "total")):
        res = call("get_cart")
        cart = res.get("cart", {})
